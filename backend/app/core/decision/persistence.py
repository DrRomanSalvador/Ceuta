...
class SQLiteDecisionStore:
    ...
    def _json(value: object, *, default: object | None = None) -> str:
        kwargs = {"sort_keys": True, "separators": (",", ":"), "ensure_ascii": False, "allow_nan": False}
...
CREATE INDEX IF NOT EXISTS idx_citation_traces_claim ON citation_traces(claim_id);"""
    ...
    def append(self, event: DecisionAuditEvent) -> None:
        ...
        try:
            ...
            self.connection.execute("INSERT INTO decision_audit(event_id,decision_id,event_type,payload_json,previous_hash,event_hash,timestamp) VALUES(?,?,?,?,?,?,?)", (event.event_id, event.decision_id, event.event_type, canonical, event.previous_hash, event.event_hash, event.timestamp))
            self.connection.commit()
        except Exception:
            self.connection.rollback()
            raise
    ...
    def record_outcome(self, outcome: DecisionOutcome) -> None:
        ...
        if existing is not None:
            ...
            return
        self.connection.execute("INSERT INTO decision_outcomes(decision_id,option_id,outcome_at,payload_json) VALUES(?,?,?,?)", (outcome.decision_id, outcome.option_id, outcome.outcome_at, payload))
        self.connection.commit()
    ...
    def record_lineage(self, lineage: DecisionLineage) -> None:
        ...
        if existing is not None:
            ...
            return
        self.connection.execute("INSERT INTO decision_lineage(decision_id,semantic_fingerprint,execution_fingerprint,payload_json) VALUES(?,?,?,?)", (lineage.decision_id, semantic, execution, payload))
        self.connection.commit()
    ...
    def record_conflict_resolution(self, decision_id: str, resolution: EvidenceResolution) -> None:
        values = (resolution.conflict_id, decision_id, resolution.disposition.value, self._json(resolution.selected_refs), resolution.rationale, resolution.policy_version)
        existing = self.connection.execute("SELECT decision_id,disposition,selected_refs_json,rationale,policy_version FROM decision_conflict_resolutions WHERE conflict_id=?", (resolution.conflict_id,)).fetchone()
        if existing is not None:
            if existing != values[1:]:
                raise RuntimeError("conflict resolution identity collision: existing resolution differs")
            return
        self.connection.execute("INSERT INTO decision_conflict_resolutions(conflict_id,decision_id,disposition,selected_refs_json,rationale,policy_version) VALUES(?,?,?,?,?,?)", values)
        self.connection.commit()
    ...
    def record_source(self, source: SourceRecord) -> None:
        ...
        existing = self.connection.execute("SELECT payload_json FROM evidence_sources WHERE source_id=?", (source.source_id,)).fetchone()
        if existing is not None:
            if existing[0] != payload:
                raise RuntimeError("source identity collision: existing source metadata differs")
            return
        self.connection.execute("INSERT INTO evidence_sources(source_id,payload_json) VALUES(?,?)", (source.source_id, payload))
        self.connection.commit()
    ...
    def record_claim_evidence_link(self, link: ClaimEvidenceLink) -> None:
        ...
        values = (link.claim_id, link.source_id, link.relation, link.excerpt_ref, int(link.supports_claim))
        existing = self.connection.execute("SELECT excerpt_ref,supports_claim FROM claim_evidence_links WHERE claim_id=? AND source_id=? AND relation=?", (link.claim_id, link.source_id, link.relation)).fetchone()
        if existing is not None:
            if existing != values[3:]:
                raise RuntimeError("claim evidence identity collision: existing link differs")
            return
        self.connection.execute("INSERT INTO claim_evidence_links(claim_id,source_id,relation,excerpt_ref,supports_claim) VALUES(?,?,?,?,?)", values)
        self.connection.commit()

    def record_citation_trace(self, trace: CitationTrace) -> None:
        values = (trace.claim_id, trace.source_id, trace.locator, trace.captured_text_hash, trace.captured_at)
        existing = self.connection.execute("SELECT captured_at FROM citation_traces WHERE claim_id=? AND source_id=? AND locator=? AND captured_text_hash=?", values[:4]).fetchone()
        if existing is not None:
            if existing[0] != trace.captured_at:
                raise RuntimeError("citation trace identity collision: captured_at differs")
            return
        self.connection.execute("INSERT INTO citation_traces(claim_id,source_id,locator,captured_text_hash,captured_at) VALUES(?,?,?,?,?)", values)
        self.connection.commit()
    ...
    def record_cycle(self, *, system_id: str, as_of: str, decision_id: str | None, option_id: str | None, disposition: str | None, lineage: Sequence[str], stages: Sequence[Mapping[str, object]]) -> None:
        ...
        values = (system_id, as_of, decision_id, option_id, disposition, lineage_json, stages_json)
        existing = self.connection.execute("SELECT decision_id,option_id,disposition,lineage_json,stages_json FROM decision_cycles WHERE system_id=? AND as_of=?", (system_id, as_of)).fetchone()
        if existing is not None:
            if existing != values[2:]:
                raise RuntimeError("decision cycle identity collision: existing snapshot differs")
            return
        self.connection.execute("INSERT INTO decision_cycles(system_id,as_of,decision_id,option_id,disposition,lineage_json,stages_json) VALUES(?,?,?,?,?,?,?)", values)
        self.connection.commit()
