...
class SQLiteDecisionStore:
    ...
    def _migrate(self)->None:
        ...
        if current>self.SCHEMA_VERSION: raise RuntimeError(f"database schema version {current} is newer than supported {self.SCHEMA_VERSION}")
        if current<1:
...
CREATE INDEX IF NOT EXISTS idx_decision_lineage_semantic ON decision_lineage(semantic_fingerprint);"""); self.connection.execute("UPDATE ceutia_schema_version SET version=2"); current=2
        if current<3:
...
CREATE INDEX IF NOT EXISTS idx_conflict_resolutions_decision ON decision_conflict_resolutions(decision_id);"""); self.connection.execute("UPDATE ceutia_schema_version SET version=3"); current=3
        if current<4:
...
CREATE INDEX IF NOT EXISTS idx_citation_traces_claim ON citation_traces(claim_id);"""); self.connection.execute("UPDATE ceutia_schema_version SET version=4")
    ...
    def lineage(self,decision_id:str)->DecisionLineage|None:
        ...
        payload=json.loads(row[0]); from .lineage import LineageNode
    ...
    def conflict_resolutions(self,decision_id:str)->tuple[EvidenceResolution,...]:
        rows=self.connection.execute("SELECT conflict_id,disposition,selected_refs_json,rationale,policy_version FROM decision_conflict_resolutions WHERE decision_id=? ORDER BY conflict_id",(decision_id,)).fetchall(); from ..evidence.conflict_resolution import ResolutionDisposition
    ...
    def record_source(self,source:SourceRecord)->None:
        self.connection.execute("INSERT OR REPLACE INTO evidence_sources(source_id,payload_json) VALUES(?,?)",(source.source_id,json.dumps(asdict(source),sort_keys=True,default=lambda value:value.value))); self.connection.commit()
    @staticmethod
    def _source_from_payload(payload:dict[str,object])->SourceRecord:
        payload=dict(payload); payload["verification"]=SourceVerification(payload["verification"]); payload["role"]=SourceRole(payload["role"]); payload["assumptions"]=tuple(payload.get("assumptions",())); payload["limitations"]=tuple(payload.get("limitations",())); return SourceRecord(**payload)
    def source(self,source_id:str)->SourceRecord|None:
        row=self.connection.execute("SELECT payload_json FROM evidence_sources WHERE source_id=?",(source_id,)).fetchone(); return None if row is None else self._source_from_payload(json.loads(row[0]))
    def source_registry(self)->SourceRegistry:
        registry=SourceRegistry(); rows=self.connection.execute("SELECT payload_json FROM evidence_sources ORDER BY source_id").fetchall()
        for row in rows: registry.register(self._source_from_payload(json.loads(row[0])))
        ...
        for row in links: registry.link_claim(ClaimEvidenceLink(row[0],row[1],row[2],row[3],bool(row[4])))
        return registry
    def record_claim_evidence_link(self,link:ClaimEvidenceLink)->None:
        if self.source(link.source_id) is None: raise KeyError(f"unknown source_id: {link.source_id}")
        self.connection.execute("INSERT OR REPLACE INTO claim_evidence_links(claim_id,source_id,relation,excerpt_ref,supports_claim) VALUES(?,?,?,?,?)",(link.claim_id,link.source_id,link.relation,link.excerpt_ref,int(link.supports_claim))); self.connection.commit()
    def record_citation_trace(self,trace:CitationTrace)->None:
        self.connection.execute("INSERT OR REPLACE INTO citation_traces(claim_id,source_id,locator,captured_text_hash,captured_at) VALUES(?,?,?,?,?)",(trace.claim_id,trace.source_id,trace.locator,trace.captured_text_hash,trace.captured_at)); self.connection.commit()
    def citation_registry(self)->CitationTraceRegistry:
        registry=CitationTraceRegistry(); rows=self.connection.execute("SELECT claim_id,source_id,locator,captured_text_hash,captured_at FROM citation_traces ORDER BY claim_id,source_id,locator,captured_text_hash").fetchall()
        for row in rows: registry.add(CitationTrace(*row))
        return registry
    def record_cycle(self,*,system_id:str,as_of:str,decision_id:str|None,option_id:str|None,disposition:str|None,lineage:Sequence[str],stages:Sequence[Mapping[str,object]])->None:
        self.connection.execute("INSERT OR REPLACE INTO decision_cycles(system_id,as_of,decision_id,option_id,disposition,lineage_json,stages_json) VALUES(?,?,?,?,?,?,?)",(system_id,as_of,decision_id,option_id,disposition,json.dumps(tuple(lineage),sort_keys=True),json.dumps(tuple(stages),sort_keys=True,default=str))); self.connection.commit()
    def cycles(self,system_id:str)->tuple[dict[str,object],...]:
        rows=self.connection.execute("SELECT system_id,as_of,decision_id,option_id,disposition,lineage_json,stages_json FROM decision_cycles WHERE system_id=? ORDER BY as_of",(system_id,)).fetchall(); return tuple({"system_id":r[0],"as_of":r[1],"decision_id":r[2],"option_id":r[3],"disposition":r[4],"lineage":tuple(json.loads(r[5])),"stages":tuple(json.loads(r[6]))} for r in rows)
    def verify_chain(self,decision_id:str)->bool:
        previous="GENESIS"; import hashlib
        for event in self.events(decision_id):
            canonical=json.dumps(dict(event.payload),sort_keys=True,default=str,separators=(",",":")); expected=hashlib.sha256(f"{previous}|{event.event_id}|{canonical}".encode()).hexdigest()
