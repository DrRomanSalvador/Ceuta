...
def _fit_logistic_calibration(logits: Sequence[float], outcomes: Sequence[int]) -> tuple[float, float, float, float, float]:
    """Unpenalized logistic calibration with observed-information covariance.

    The scientific estimand is the standard calibration intercept/slope.
    Regularization is deliberately absent. Complete separation, singular
    information and numerical non-convergence are explicit failures.
    """
    ...
    for _ in range(100):
        ...
        for x, y in zip(logits, outcomes):
            ...
            ll += y * log(max(p, _EPS)) + (1-y) * log(max(1-p, _EPS))
        det = h00*h11 - h01*h01
        ...
        if max(abs(g0), abs(g1)) <= 1e-12:
            return a, b, h11/det, -h01/det, h00/det
        d0 = (g0*h11 - g1*h01)/det
        d1 = (h00*g1 - h01*g0)/det
        accepted = False
        factor = 1.0
        for _ in range(80):
            ca, cb = a-factor*d0, b-factor*d1
            cll = 0.0
            for x, y in zip(logits, outcomes):
                q = _sigmoid(ca + cb*x)
                cll += y*log(max(q,_EPS)) + (1-y)*log(max(1-q,_EPS))
            if cll >= ll:
                a, b = ca, cb; accepted = True; break
            factor *= 0.5
        if not accepted:
...
def _wilson_interval(successes: int, n: int) -> tuple[float, float]:
    ...
    z, p = _Z95, successes/n
    z2 = z*z
    d = 1.0 + z2/n
    c = (p + z2/(2*n))/d
    h = z*sqrt(p*(1-p)/n + z2/(4*n*n))/d
...
class PredictiveValidation:
    ...
    def calibration_report(predictions: Sequence[float], outcomes: Sequence[int], *, bins: int = 10) -> CalibrationReport:
        ...
        skill=None if baseline == 0.0 else 1-brier/baseline
        logits=tuple(_logit(pi) for pi in p)
        intercept,slope,c00,_,c11=_fit_logistic_calibration(logits,y)
        ise,sse=sqrt(max(c00,0.0)),sqrt(max(c11,0.0))
        intercept_ci=(intercept-_Z95*ise,intercept+_Z95*ise)
        slope_ci=(slope-_Z95*sse,slope+_Z95*sse)
        expected=sum(p); observed=sum(y)
        oe=None if expected <= 0 else observed/expected
        oe_ci=None
        if observed > 0 and expected > 0:
            se_log=1/sqrt(observed)
            oe_ci=(oe*exp(-_Z95*se_log),oe*exp(_Z95*se_log))
        errors=[]
        for k in range(bins):
            lo,hi=k/bins,(k+1)/bins
            idx=[i for i,pi in enumerate(p) if lo<=pi<hi or (k==bins-1 and pi==1.0)]
            if idx: errors.append(sum(y[i]-p[i] for i in idx)/len(idx))
        return CalibrationReport(brier,log_loss,intercept,slope,ise,sse,intercept_ci,slope_ci,oe,oe_ci,skill,tuple(errors),n,prevalence)

    @staticmethod
    def discrimination(predictions: Sequence[float], outcomes: Sequence[int]) -> float | None:
        if len(predictions) != len(outcomes) or not predictions:
            raise ValueError("predictions and outcomes must be non-empty and have equal length")
        if any(o not in (0,1) for o in outcomes): raise ValueError("binary outcomes must be 0 or 1")
        return _auc(predictions,outcomes)

    @staticmethod
    def prevalence_interval(outcomes: Sequence[int]) -> tuple[float,float]:
        if not outcomes or any(o not in (0,1) for o in outcomes): raise ValueError("outcomes must be non-empty and binary")
...
class ValidationAssessment:
    ...
    def scientifically_strong(self) -> bool:
        return (self.externally_validated and self.temporal_holdout and self.development_data_disjoint
                and self.calibration is not None and self.calibration.calibration_intercept_ci is not None
