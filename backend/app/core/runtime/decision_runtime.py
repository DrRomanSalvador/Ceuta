...
class DecisionRuntime:
    def __init__(self, decision_system: DecisionSystem | None = None, voi: ValueOfInformationEngine | None = None, decision_engine: DecisionEngine | None = None, control_plane: DecisionControlPlane | None = None):
    ...
    @staticmethod
    def _manifest(decision_id: str, provenance: Sequence[str], assumptions: Sequence[str], scenario_refs: Sequence[str]) -> DecisionManifest:
        ...
        configuration_hash = sha256(("|".join(sorted(provenance)) + "|" + "|".join(sorted(assumptions))).encode("utf-8")).hexdigest()
        return DecisionManifest(decision_id=decision_id, state_refs=tuple(buckets["state"] or (f"decision-state:{decision_id}",)), evidence_refs=tuple(buckets["evidence"]), model_refs=tuple(buckets["model"] or ("declared-decision-model",)), hypothesis_refs=tuple(buckets["hypothesis"]), transformation_refs=tuple(buckets["transformation"]), assumption_refs=tuple(assumptions), scenario_refs=tuple(buckets["scenario"]), utility_definition_ref="decision-utility:v1", constraint_refs=tuple(buckets["constraint"]), policy_version="1.0", configuration_hash=configuration_hash, code_revision="runtime-control-plane-v1", created_at="runtime")

    def decide(self, context: DecisionContext, options: Sequence[DecisionOption], escalation: EscalationAssessment, *, mode: DecisionMode = DecisionMode.ROBUST, provenance: Sequence[str] = (), purpose: str = "decision", restricted: bool = False) -> DecisionRuntimeResult:
    ...
    def decide_scenarios(self, *, decision_id: str, options: Sequence[ActionAlternative], escalation: EscalationAssessment, observable: bool, identifiable: bool, calibrated: bool, model_valid: bool, causal_identified: bool = True, assumptions_satisfied: bool = True, mode: DecisionMode = DecisionMode.ROBUST, max_harm: float | None = None, assumptions: Sequence[str] = (), provenance: Sequence[str] = (), reevaluation_triggers: Sequence[str] = (), purpose: str = "decision", restricted: bool = False) -> DecisionCycleResult:
        ...
        return self.engine.evaluate(decision_id=decision_id, options=options, gate=gate, mode=mode, max_harm=max_harm, assumptions=(*assumptions, *("human review required before execution",) if control.disposition is ControlDisposition.HUMAN_REVIEW else ()), provenance=(*provenance, f"audit:{control.audit_event_id}"), reevaluation_triggers=reevaluation_triggers)
