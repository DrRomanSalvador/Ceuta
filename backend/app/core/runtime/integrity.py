...
class ProvenanceEnvelope: record_id:str; source_id:str; origin_id:str; content_hash:str; parent_hashes:tuple[str,...]=(); trust_score:float=1.0
@dataclass(frozen=True, slots=True)
class IntegrityAssessment: valid:bool; reasons:tuple[str,...]; chain_hash:str
@dataclass(frozen=True, slots=True)
class PrivacyPolicy: allowed_fields:frozenset[str]; minimum_aggregation_size:int=1; purpose:str="inference"
class IntegrityEngine:
    def verify(self,envelope:ProvenanceEnvelope,payload:bytes)->IntegrityAssessment:
        ...
        chain=sha256((envelope.record_id+envelope.content_hash+"|".join(envelope.parent_hashes)).encode()).hexdigest(); return IntegrityAssessment(not reasons,tuple(reasons),chain)
    def enforce_policy(self,fields:Sequence[str],policy:PrivacyPolicy,population_size:int)->tuple[str,...]:
        ...
        return tuple(fields)
def validate_evidence_set(evidence:Sequence[DecisionEvidence], *, source_registry:SourceRegistry|None=None, as_of:datetime|None=None)->None:
    """Reject malformed, unregistered or temporally invalid decision evidence."""
    seen:set[str]=set(); point=as_of or datetime.now(timezone.utc)
    if point.tzinfo is None or point.utcoffset() is None: raise ValueError("evidence validation point must be timezone-aware")
    for item in evidence:
        ...
        if not item.temporal.valid_at(point): raise ValueError(f"evidence is not valid at decision time: {item.evidence_id}")
        if not item.temporal.known_at(point): raise ValueError(f"evidence was not known at decision time: {item.evidence_id}")
...
def propagate_uncertainty(states:Iterable[UncertaintyState],*,method:str="conservative-max")->UncertaintyState:
    ...
    value=sum(x.value for x in items)/len(items) if method=="mean" else max(x.value for x in items); return UncertaintyState(value,source_refs=tuple(ref for state in items for ref in state.source_refs),method=method)
