...
def ensure_prediction_outcome_schema(connection: sqlite3.Connection) -> None:
    ...
    for name, definition in migrations.items():
        if columns and name not in columns: connection.execute(f"ALTER TABLE scientific_prediction_outcomes ADD COLUMN {name} {definition}")
    if columns and "action_id" not in columns: connection.execute("ALTER TABLE scientific_prediction_outcomes ADD COLUMN action_id TEXT NOT NULL DEFAULT ''")
...
def _horizon_delta(horizon: str) -> timedelta:
    ...
    if compact:
        amount = float(compact.group("value")); return {"S": timedelta(seconds=amount), "M": timedelta(minutes=amount), "H": timedelta(hours=amount), "D": timedelta(days=amount)}[compact.group("unit")]
...
def _prediction_fingerprint(payload: dict[str, Any]) -> str:
    ...
    return sha256(json.dumps(payload, sort_keys=True, separators=(",", ":"), ensure_ascii=False, allow_nan=False).encode("utf-8")).hexdigest()
...
def record_prediction_outcome(connection: sqlite3.Connection, *, prediction_id: str, decision_id: str, action_id: str, outcome_id: str, target: str, outcome_time: datetime, observed: int, provenance: tuple[str, ...], source_id: str, source_version: str, observation_time: datetime, availability_time: datetime, ascertainment_time: datetime, revision_id: str, measurement_process_id: str, outcome_definition_version: str, transformation_id: str, censoring_status: str = "NONE", missingness_status: str = "OBSERVED", selection_status: str = "NONE", intervention_exposure_id: str | None = None) -> dict[str, Any]:
    ...
    if any(not value for value in required) or not provenance: raise ValueError("prediction outcome identity, provenance and ascertainment identity are required")
    if observed not in (0, 1): raise ValueError("observed outcome must be 0 or 1")
    if censoring_status not in _ALLOWED_CENSORING or missingness_status not in _ALLOWED_MISSINGNESS or selection_status not in _ALLOWED_SELECTION: raise ValueError("unsupported outcome ascertainment status")
    outcome_time, observation_time, availability_time, ascertainment_time = (_parse_aware(value, name) for value, name in ((outcome_time, "outcome_time"), (observation_time, "observation_time"), (availability_time, "availability_time"), (ascertainment_time, "ascertainment_time")))
    if observation_time > availability_time or availability_time > ascertainment_time: raise ValueError("outcome observation, availability and ascertainment times must be ordered")
    if missingness_status != "OBSERVED" or censoring_status != "NONE" or selection_status != "NONE": raise ValueError("non-observed, censored, or selected outcomes are not eligible for binary scoring")
    owns_transaction = not connection.in_transaction
    ...
    try:
        ...
        if prediction is None: raise KeyError("prediction_id not found")
        stored_decision_id, available_at_raw, payload_raw, stored_fingerprint = prediction
        if stored_decision_id != decision_id: raise ValueError("prediction does not belong to decision_id")
        payload = json.loads(payload_raw)
        if _prediction_fingerprint(payload) != str(stored_fingerprint): raise RuntimeError("prediction persistence integrity mismatch")
        probability = float(payload["probability"])
        if not math.isfinite(probability) or not 0.0 <= probability <= 1.0: raise ValueError("persisted prediction probability is outside [0,1]")
        if str(payload["target"]) != target: raise ValueError("outcome target does not match prediction target")
        ...
        if outcome_time > datetime.now(timezone.utc): raise ValueError("outcome cannot be in the future")
        if outcome_time < available_at: raise ValueError("outcome cannot precede prediction availability")
        if outcome_time < origin_time + _horizon_delta(str(payload["horizon"])): raise ValueError("outcome precedes prediction target time; prospective outcome is not yet eligible")
        if availability_time < available_at: raise ValueError("outcome availability cannot precede prediction availability")
        if ascertainment_time < availability_time: raise ValueError("outcome ascertainment cannot precede outcome availability")
        brier_error = float((probability - observed) ** 2); log_loss_error = _log_loss(probability, observed); canonical_provenance = tuple(dict.fromkeys(str(item) for item in provenance))
        ...
        if row is not None:
            if row != values: raise RuntimeError("prediction outcome identity collision: existing outcome differs")
            if owns_transaction: connection.commit()
