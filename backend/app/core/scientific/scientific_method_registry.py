...
class ScientificMethodRegistry:
    ...
    @staticmethod
    def _decode_verified(payload: str, fingerprint: str) -> ScientificMethodRelease:
        if sha256(payload.encode()).hexdigest() != str(fingerprint):
            raise RuntimeError("scientific method registry integrity mismatch")
        data = json.loads(payload)
        data["preprocessing"] = tuple(data["preprocessing"])
        data["assumptions"] = tuple(data["assumptions"])
        data["source_ids"] = tuple(data["source_ids"])
        release = ScientificMethodRelease(**data)
        if release.fingerprint != str(fingerprint):
            raise RuntimeError("scientific method registry payload identity mismatch")
        return release

    def get(self, method_id: str, version: str) -> ScientificMethodRelease:
        with sqlite3.connect(self.storage_path) as db:
            row = db.execute("SELECT payload,fingerprint FROM scientific_method_releases WHERE method_id=? AND version=?", (method_id, version)).fetchone()
        if not row:
            raise KeyError((method_id, version))
        return self._decode_verified(str(row[0]), str(row[1]))

    def all(self) -> tuple[ScientificMethodRelease, ...]:
        with sqlite3.connect(self.storage_path) as db:
            rows = db.execute("SELECT payload,fingerprint FROM scientific_method_releases ORDER BY method_id,version").fetchall()
        return tuple(self._decode_verified(str(payload), str(fingerprint)) for payload, fingerprint in rows)
