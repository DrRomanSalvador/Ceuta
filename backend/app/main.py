...
from app.core.scientific.scientific_routes import router as scientific_router
...
def load_runtime_config():
    return RuntimeConfig(
        _read_host(),
        _read_port(),
        os.getenv("CEUTIA_CODE_REVISION", "").strip(),
        os.getenv("CEUTIA_DECISION_DB", "").strip(),
    )
...
def _readiness():
    ...
    if not os.getenv("CEUTIA_SERPIENTE_TRANSPORT_SECRET", "").strip():
        missing.append("CEUTIA_SERPIENTE_TRANSPORT_SECRET")
    if missing:
...
class DecisionRequest(BaseModel):
    ...
    serpiente_prediction: dict[str, Any] | None = None


def _final_epistemic_assessment(payload, evidence, prediction=None):
    provenance_refs = tuple(ref for item in evidence for ref in item.provenance_refs)
    if prediction is not None:
        provenance_refs = tuple(dict.fromkeys(provenance_refs + tuple(prediction.provenance) + (f"serpiente:prediction:{prediction.prediction_id}",)))
    external_evidence_refs = tuple(item.evidence_id for item in evidence)
    if prediction is not None:
        external_evidence_refs = tuple(dict.fromkeys(external_evidence_refs + (f"serpiente:{prediction.prediction_id}",)))
    ...
    condition = FalsificationCondition(
        ...
        expected_observation="cited evidence and any SERPIENTE forecast remain temporally valid, provenance-complete and decision-eligible",
        falsifying_observation="material contradiction, provenance failure, temporal invalidity or future SERPIENTE prediction is detected",
        independent_evidence_refs=independent_refs,
        assumptions=("bitemporal validity is enforced before decision execution", "SERPIENTE forecasts are predictive rather than causal"),
        testable=bool(independent_refs),
    )
    anchor = RealityAnchorAssessment(
        target_id=payload.decision_id,
        status=FalsifiabilityStatus.PARTIAL,
        conditions=(condition,),
        external_evidence_refs=external_evidence_refs,
        common_mode_dependencies=(),
        divergence_score=0.0,
        global_model_doubt=global_doubt,
        reasons=("reality anchoring is operationally represented but not independently established",),
    )
    contract = EpistemicContract(
        contract_id=f"{payload.decision_id}:epistemic-contract",
        semantic_meaning=f"decision support for: {payload.purpose}",
        ...
        causal_interpretation="non-causal decision support; SERPIENTE prediction is not a causal effect",
        evidence_status="runtime decision evidence plus optional SERPIENTE forecast",
        calibration_conditions=("prospective effectiveness is not inferred from execution",),
        validity_domain=payload.horizon,
        uncertainty_semantics="conservative maximum adversarial evidence risk plus explicit predictive uncertainty",
        dependency_refs=tuple(payload.model_refs) + ((f"serpiente-prediction:{prediction.prediction_id}",) if prediction is not None else ()),
    )
    ...
    self_model = EpistemicSelfModel(
        version="runtime-1",
        assumptions=("decision inputs are schema-validated", "evidence provenance is explicit", "SERPIENTE output is treated as prediction, not causation"),
        limitations=("no prospective effectiveness is established", "external evidence quality is not equivalent to causal identification", "SERPIENTE forecast validity depends on its point-in-time fingerprint and calibration evidence"),
...
async def evaluate_decision(payload: DecisionRequest, request: Request):
    ...
    if not configured_key:
        return JSONResponse(status_code=503, content={"status": STATUS_NOT_READY, "reason": "high-impact decision endpoint is not provisioned with CEUTIA_DECISION_API_KEY"})
    if not provided_key or not hmac.compare_digest(provided_key, configured_key):
        return JSONResponse(status_code=401, content={"error": "unauthorized", "reason": "valid decision API key required"})
    state = _readiness()
    if not state.ready:
        return JSONResponse(status_code=503, content={"status": STATUS_NOT_READY, "reason": state.reason})
    decision_time = datetime.now(timezone.utc)
    prediction = None
    if payload.serpiente_prediction is not None:
        acceptance = consume_serpiente_prediction(payload.serpiente_prediction, decision_time=decision_time)
        prediction = acceptance.prediction
        if prediction is not None:
            store = SQLiteDecisionStore(RUNTIME_CONFIG.decision_db)
            try:
                record_prediction(store.connection, payload.serpiente_prediction, decision_id=payload.decision_id)
            finally:
                store.close()
        if not acceptance.accepted:
            return JSONResponse(status_code=422, content={"decision_id": payload.decision_id, "disposition": "abstain", "control_reason": acceptance.reason, "serpiente_prediction_id": prediction.prediction_id if prediction else None})
    for option in payload.options:
        validate_scenario_probabilities([scenario.probability for scenario in option.scenarios])
    store = SQLiteDecisionStore(RUNTIME_CONFIG.decision_db)
    try:
        configuration = ConfigurationProvenance(configuration_id="ceutia-decision-runtime", version="1", source_refs=("environment:decision-runtime",), values={"risk_class": payload.risk_class.value, "mode": payload.mode.value})
        ...
        for item in payload.evidence:
            ...
            assessment = EvidenceAssessment(evidence_id=item.evidence_id, source_id=item.source_id, base_weight=item.base_weight, adversarial_risk=item.adversarial_risk, contradiction_weight=item.contradiction_weight, independent_origin=item.independent_origin, disposition=item.disposition)
        ...
        try:
            validate_evidence_set(evidence, source_registry=store.source_registry(), as_of=decision_time)
        except ValueError as exc:
            return JSONResponse(status_code=422, content={"decision_id": payload.decision_id, "disposition": "abstain", "control_reason": "evidence integrity validation failed", "reason": str(exc)})
        try:
            final_epistemic = _final_epistemic_assessment(payload, evidence, prediction)
        except ValueError as exc:
            return JSONResponse(status_code=422, content={"decision_id": payload.decision_id, "disposition": "abstain", "control_reason": "cross-system prediction integrity validation failed", "reason": str(exc)})
        gate_event = DecisionAuditChain(store).append(payload.decision_id, "final_epistemic_gate", {"validity": final_epistemic.validity.value, "composition": final_epistemic.composition.value, "reasons": final_epistemic.reasons, "anchor_status": final_epistemic.reality_anchor.status.value, "anchor_established": False, "serpiente_prediction_id": prediction.prediction_id if prediction else None})
        if final_epistemic.validity is not SystemValidity.SUPPORTED:
            return JSONResponse(status_code=409, content={"decision_id": payload.decision_id, "disposition": "abstain", "control_reason": "final epistemic controller suspended decision eligibility", "epistemic_validity": final_epistemic.validity.value, "epistemic_composition": final_epistemic.composition.value, "epistemic_reasons": final_epistemic.reasons, "audit_event_id": gate_event.event_id})
        if final_epistemic.composition is not EpistemicIntegrityStatus.PRESERVED:
            return JSONResponse(status_code=409, content={"decision_id": payload.decision_id, "disposition": "abstain", "control_reason": "final epistemic composition is not preserved", "epistemic_composition": final_epistemic.composition.value, "audit_event_id": gate_event.event_id})
        evidence_store = DecisionEvidenceStore(store)
        for item in evidence:
            evidence_store.record(item)
        options = tuple(DecisionOption(item.option_id, tuple(ScenarioOutcome(s.scenario_id, s.probability, s.utility, s.harm) for s in item.scenarios), item.resource_cost, item.uncertainty) for item in payload.options)
        ...
        result = lifecycle.execute(context, options, evidence, state_refs=payload.state_refs, hypothesis_refs=payload.hypothesis_refs, model_refs=model_refs, prediction_refs=((f"serpiente-prediction:{prediction.prediction_id}",) if prediction else ()), scenario_refs=scenario_refs, assumption_refs=payload.assumptions, transformation_refs=payload.transformation_refs, constraint_refs=payload.constraint_refs, purpose=payload.purpose, restricted=payload.restricted, output_visibility=payload.output_visibility, mode=payload.mode, model_releases=model_releases, at=decision_time)
        return {"decision_id": result.decision_id, "disposition": result.disposition.value, "recommendation": {"option_id": result.recommendation.option_id, "score": result.recommendation.score, "expected_utility": result.recommendation.expected_utility, "expected_harm": result.recommendation.expected_harm, "maximum_regret": result.recommendation.maximum_regret, "reasons": result.recommendation.reasons}, "control_reason": result.control_reason, "audit_event_id": result.audit_event_id, "epistemic_gate_event_id": gate_event.event_id, "serpiente_prediction_id": prediction.prediction_id if prediction else None, "lineage_fingerprint": result.lineage.semantic_fingerprint(), "uncertainty": result.uncertainty.value, "degraded_reasons": result.degraded_reasons}
    finally:
        store.close()
...
async def diagnostics_config():
    state = _readiness()
    return {"status": "valid" if state.ready else "incomplete", "host_configured": bool(RUNTIME_CONFIG.host), "port_valid": MIN_PORT <= RUNTIME_CONFIG.port <= MAX_PORT, "code_revision_configured": bool(RUNTIME_CONFIG.code_revision), "decision_persistence_configured": bool(RUNTIME_CONFIG.decision_db), "decision_api_key_configured": bool(os.getenv("CEUTIA_DECISION_API_KEY", "")), "serpiente_transport_secret_configured": bool(os.getenv("CEUTIA_SERPIENTE_TRANSPORT_SECRET", ""))}
