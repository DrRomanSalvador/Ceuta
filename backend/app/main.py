...
def _final_epistemic_assessment(payload,evidence):
    provenance_refs=tuple(ref for item in evidence for ref in item.provenance_refs); external_evidence_refs=tuple(item.evidence_id for item in evidence); independent_refs=tuple(item.evidence_id for item in evidence if item.assessment.independent_origin); global_doubt=(not evidence or not independent_refs or any(item.assessment.disposition is not EvidenceDisposition.ACCEPT for item in evidence) or any(item.assessment.contradiction_weight>0.5 for item in evidence))
...
async def evaluate_decision(payload:DecisionRequest,request:Request):
    ...
    try:
        ...
        validate_evidence_set(evidence,source_registry=store.source_registry()); final_epistemic=_final_epistemic_assessment(payload,evidence)
        ...
        if final_epistemic.composition is not EpistemicIntegrityStatus.PRESERVED: return JSONResponse(status_code=409,content={"decision_id":payload.decision_id,"disposition":"abstain","control_reason":"final epistemic composition is not preserved","epistemic_composition":final_epistemic.composition.value})
