...
class DecisionRequest(BaseModel):
    decision_id:str=Field(min_length=1); decision_maker:str=Field(min_length=1); horizon:str=Field(min_length=1); purpose:str=Field(min_length=1); risk_class:DecisionRisk=DecisionRisk.LOW; mode:DecisionMode=DecisionMode.ROBUST; state_refs:list[str]=Field(min_length=1); evidence:list[DecisionEvidenceRequest]=Field(min_length=1); options:list[DecisionOptionRequest]=Field(min_length=1); assumptions:list[str]=Field(default_factory=list); hypothesis_refs:list[str]=Field(default_factory=list); model_refs:list[str]=Field(default_factory=list); scenario_refs:list[str]=Field(default_factory=list); transformation_refs:list[str]=Field(default_factory=list); constraint_refs:list[str]=Field(default_factory=list); restricted:bool=False; output_visibility:InformationVisibility=InformationVisibility.PUBLIC; serpiente_prediction:dict[str,Any]|None=None
...
async def evaluate_decision(payload:DecisionRequest,request:Request):
    ...
    if payload.serpiente_prediction is not None:
        ...
        if prediction is not None:
            ...
            try: record_prediction(store.connection,payload.serpiente_prediction,decision_id=payload.decision_id)
            finally: store.close()
    ...
    try:
        ...
        try: validate_evidence_set(evidence,source_registry=store.source_registry(),as_of=decision_time)
        ...
        try: final_epistemic=_final_epistemic_assessment(payload,evidence,prediction)
