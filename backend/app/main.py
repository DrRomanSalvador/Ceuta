...
class RuntimeConfig:
    host:str; port:int; code_revision:str; decision_db:str
...
def load_runtime_config():
    return RuntimeConfig(_read_host(),_read_port(),os.getenv("CEUTIA_CODE_REVISION","").strip(),os.getenv("CEUTIA_DECISION_DB","").strip())
...
class DecisionScenarioRequest(BaseModel):
    scenario_id:str=Field(min_length=1); probability:float=Field(ge=0.0,le=1.0); utility:float; harm:float
class DecisionOptionRequest(BaseModel):
    option_id:str=Field(min_length=1); scenarios:list[DecisionScenarioRequest]=Field(min_length=1); resource_cost:float=Field(default=0.0,ge=0.0); uncertainty:float=Field(default=0.0,ge=0.0,le=1.0)
...
class DecisionRequest(BaseModel):
    decision_id:str=Field(min_length=1); decision_maker:str=Field(min_length=1); horizon:str=Field(min_length=1); purpose:str=Field(min_length=1); risk_class:DecisionRisk=DecisionRisk.LOW; mode:DecisionMode=DecisionMode.ROBUST; state_refs:list[str]=Field(min_length=1); evidence:list[DecisionEvidenceRequest]=Field(min_length=1); options:list[DecisionOptionRequest]=Field(min_length=1); assumptions:list[str]=Field(default_factory=list); hypothesis_refs:list[str]=Field(default_factory=list); model_refs:list[str]=Field(default_factory=list); scenario_refs:list[str]=Field(default_factory=list); transformation_refs:list[str]=Field(default_factory=list); constraint_refs:list[str]=Field(default_factory=list); restricted:bool=False; output_visibility:InformationVisibility=InformationVisibility.PUBLIC

def _final_epistemic_assessment(payload,evidence):
    provenance_refs=tuple(ref for item in evidence for ref in item.provenance_refs); external_evidence_refs=tuple(item.evidence_id for item in evidence); independent_refs=tuple(item.evidence_id for item in evidence if item.assessment.independent_origin)
    ...
    condition=FalsificationCondition(condition_id=f"{payload.decision_id}:evidence-integrity",target_id=payload.decision_id,expected_observation="cited evidence remains temporally valid, provenance-complete and decision-eligible",falsifying_observation="material contradiction, provenance failure or temporal invalidity is detected",independent_evidence_refs=independent_refs,assumptions=("bitemporal evidence validation is enforced before decision execution",),testable=bool(independent_refs))
    ...
    self_model=EpistemicSelfModel(version="runtime-1",assumptions=("decision inputs are schema-validated","evidence provenance is explicit"),limitations=("no prospective effectiveness is established","external evidence quality is not equivalent to causal identification"),identification_limits=("causal effect is not identified by this decision endpoint",),ontology_status=OntologyStatus.NORMAL,ontology_version="runtime-1",unexplained_signals=(),global_validity=SystemValidity.DOUBT if global_doubt else SystemValidity.SUPPORTED)
...
async def evaluate_decision(payload:DecisionRequest):
    ...
    try:
        ...
        lifecycle=DecisionLifecycleEngine(store,code_revision=RUNTIME_CONFIG.code_revision,configuration=configuration); evidence=[]
        ...
        scenario_refs=tuple(payload.scenario_refs) or tuple(s.scenario_id for option in options for s in option.outcomes); model_refs=tuple(payload.model_refs); model_releases={}
        if not model_refs:
            ...
            deterministic=replace(deterministic,release_hash=deterministic.fingerprint()); model_releases[deterministic.model_id]=deterministic; model_refs=(deterministic.model_id,)
...
async def diagnostics_config():
    state=_readiness()
    return {"status":"valid" if state.ready else "incomplete","host_configured":bool(RUNTIME_CONFIG.host),"port_valid":MIN_PORT<=RUNTIME_CONFIG.port<=MAX_PORT,"code_revision_configured":bool(RUNTIME_CONFIG.code_revision),"decision_persistence_configured":bool(RUNTIME_CONFIG.decision_db)}
