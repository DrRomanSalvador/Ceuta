...
class UniversalExecutionRuntime:
    """Repository-side execution control shared by every admitted mission.

    This runtime enforces state transitions, coherence-gate schemas, atomic persistence,
    idempotent checkpoints and compare-and-swap writes. It does not pretend to be an
    external process supervisor or scheduler; those capabilities remain deployment
    boundaries unless a host explicitly routes execution through this runtime.
    """
    ...
    def current_version(self) -> str:
        state = self.load()
        return str(state["state_version"])
    ...
    def checkpoint(self, checkpoint: Checkpoint, *, expected_version: str | None = None) -> str:
        """Write an idempotent checkpoint; conflicting reuse of an id fails closed."""
    ...
    def complete_task(
        ...
        task_id: str,
        *,
        post_write_gate: CoherenceGateResult,
        result_ref: str,
    ...
    def decide_continuation(self) -> str:
        ...
        if any(t["state"] == "EXECUTABLE_NOW" for t in tasks):
            return "CONTINUE"
        if any(t["state"] == "RUNNING" for t in tasks):
            return "CONTINUE"
        if any(t["state"] == "BLOCKED_EXTERNAL" for t in tasks):
            return "WAIT"
        if any(t["state"] == "BLOCKED_INTERNAL" for t in tasks):
            return "CONTINUE"
    ...
    def fixed_point(self) -> bool:
        ...
        if self.decide_continuation() != "MISSION_COMPLETE_CANDIDATE":
            return False
        required = state.get("fixed_point_requirements", {})
    ...
    def record_policy_change(self, *, previous_version: str, new_version: str, reason: str, author: str, evidence: Sequence[str], impact: str, compatibility: str, validation: str, timestamp: str | None = None) -> str:
        event = {
            "event_type": "POLICY_CHANGE_EVENT",
    ...
    def pre_write_coherence_gate(*, checks: Mapping[str, bool], evidence_refs: Sequence[str]) -> CoherenceGateResult:
        required = {"contradiction", "duplication", "complexity", "degradation", "regression", "integration", "maintainability", "security", "persistence"}
        missing = sorted(required - set(checks))
        failures = tuple(sorted(key for key in required if key in checks and not checks[key]))
        if missing:
            failures = failures + tuple(f"MISSING:{key}" for key in missing)
        passed = not failures and bool(evidence_refs)
        if not evidence_refs:
            failures = failures + ("MISSING:EVIDENCE_REFS",)
        return CoherenceGateResult(passed=passed, checks=dict(checks), evidence_refs=tuple(evidence_refs), failures=failures)
    ...
    def _empty_state() -> dict[str, Any]:
        state: dict[str, Any] = {
            "schema_version": UniversalExecutionRuntime.SCHEMA_VERSION,
        ...
        state["state_version"] = UniversalExecutionRuntime._version(state)
        return state

    @staticmethod
    def _next_version(state: Mapping[str, Any]) -> str:
        payload = dict(state)
        payload.pop("state_version", None)
        return UniversalExecutionRuntime._version(payload)
    ...
    @staticmethod
    def _write(state: Mapping[str, Any], path: Path | None = None) -> None:
        target = path
        if target is None:
            raise ExecutionControlError("INTERNAL_WRITE_TARGET_MISSING")
    ...
    def _write(self, state: Mapping[str, Any]) -> None:  # type: ignore[override]
        self.__class__._write.__wrapped__(state) if hasattr(self.__class__._write, "__wrapped__") else None
