...
def payload(*, independent: bool = True, transformations: list[str] | None = None) -> DecisionRequest:
    return DecisionRequest(decision_id="d1",decision_maker="owner",horizon="24h",purpose="territorial decision support",state_refs=["state-1"],evidence=[{"evidence_id":"e1","source_id":"s1","claim_id":"c1","content_hash":"a"*64,"valid_from":datetime(2026,9,14,tzinfo=timezone.utc),"recorded_from":datetime(2026,9,14,tzinfo=timezone.utc),"base_weight":1.0,"adversarial_risk":0.0,"contradiction_weight":0.0,"independent_origin":independent,"disposition":EvidenceDisposition.ACCEPT,"provenance_refs":["source:s1"],"visibility":InformationVisibility.PUBLIC}],options=[{"option_id":"o1","scenarios":[{"scenario_id":"sc1","probability":1.0,"utility":1.0,"harm":0.0}]}],transformation_refs=transformations or ["t1"])

def evidence_from_request(request: DecisionRequest) -> list[DecisionEvidence]:
    item=request.evidence[0]; temporal=BitemporalRef(item.valid_from,item.valid_until,item.recorded_from,item.recorded_until,item.evidence_id); assessment=EvidenceAssessment(evidence_id=item.evidence_id,source_id=item.source_id,base_weight=item.base_weight,adversarial_risk=item.adversarial_risk,contradiction_weight=item.contradiction_weight,independent_origin=item.independent_origin,disposition=item.disposition)
...
def test_decision_endpoint_rejects_unregistered_source(tmp_path,monkeypatch):
    monkeypatch.setattr(main_module,"RUNTIME_CONFIG",RuntimeConfig(host="127.0.0.1",port=8000,code_revision="test-revision",decision_db=str(tmp_path/"decision.sqlite3"))); monkeypatch.setenv("CEUTIA_DECISION_API_KEY","test-key"); response=TestClient(app).post("/decision/evaluate",headers={"X-CeutIA-Decision-Key":"test-key"},json=payload().model_dump(mode="json")); assert response.status_code==422; assert response.json()["disposition"]=="abstain"
