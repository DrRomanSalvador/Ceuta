...
def test_nested_temporal_cv_keeps_selection_inside_outer_training():
    records = _records()
    outer_design = strict_rolling_origin(records, initial_train_duration=2, test_duration=1, step=1, purge_gap=0, expanding=True)
    inner_design = strict_rolling_origin(records, initial_train_duration=1, test_duration=1, step=1, purge_gap=0, expanding=True)
...
def test_strict_temporal_design_has_no_overlap():
    records = _records()
    folds = strict_rolling_origin(records, initial_train_duration=1, test_duration=1, step=1, purge_gap=0)
    for fold in folds:
        assert max(records[i].time for i in fold.train_indices) < min(records[i].time for i in fold.test_indices)


def test_temporal_calibration_rejects_non_identifiable_fold():
    records = _records()
    folds = strict_rolling_origin(records, initial_train_duration=1, test_duration=1, step=1, purge_gap=0)
    with pytest.raises(ValueError, match="Hessian is non-identifiable|non-identifiable"):
        temporal_calibration_robust(records, folds, replicates=500, seed=13)


def test_transport_validation_does_not_refit_target():
    source = _records()
    target = [LongitudinalRecord("t1", 1, 0, 0.2), LongitudinalRecord("t2", 1, 1, 0.8), LongitudinalRecord("t3", 1, 0, 0.3)]
