...
def _records():
    return [
        ...
        LongitudinalRecord("b", 1, 0, 0.20), LongitudinalRecord("b", 2, 1, 0.35), LongitudinalRecord("b", 3, 1, 0.80),
        LongitudinalRecord("c", 1, 0, 0.10), LongitudinalRecord("c", 2, 1, 0.70), LongitudinalRecord("c", 3, 1, 0.90),
        LongitudinalRecord("d", 1, 0, 0.15), LongitudinalRecord("d", 2, 0, 0.55), LongitudinalRecord("d", 3, 0, 0.65),
    ]


def test_cluster_bootstrap_preserves_declared_entity_unit():
    records = _records()
    result = blocked_or_cluster_bootstrap(records, lambda rows: sum(r.outcome for r in rows) / len(rows), spec=ResampleSpec(unit="entity", seed=17), replicates=200)
...
def test_rolling_origin_has_strict_temporal_separation():
    records = _records()
    folds = rolling_origin_splits(records, initial_train_time=1.0, test_horizon=1.0, step=1.0, gap=0.0)
    assert folds
    for fold in folds:
...
def test_optimism_correction_is_not_apparent_performance():
    records = _records()
    result = optimism_corrected_bootstrap(records, lambda train, test: -sum((r.prediction-r.outcome)**2 for r in test)/len(test), spec=ResampleSpec(unit="entity", seed=3), replicates=200)
