...
CONTRIBUTION_PATH=MISSION_DIR/"MISSION_CONTRIBUTION_REGISTRY.json"; CONTRADICTION_PATH=MISSION_DIR/"MISSION_CONTRADICTION_REGISTRY.json"
...
def validate_repository_layout():
    for p in (MEMORY_PATH,RECONCILIATION_PATH,CURRENT_RECONCILIATION_PATH,CONTROL_PLANE_STATE_PATH,STATE_MACHINE_PATH,HANDOFF_REGISTRY_PATH,REGISTRY_PATH,QUEUE_PATH,EVENT_LOG_PATH,CLAIMS_PATH,CONTRIBUTION_PATH,CONTRADICTION_PATH,CONSTITUTION_PATH,SHARED_STANDARD_PATH):
...
def validate_control_plane():
    try:
        ...
        from event_log import load_jsonl,validate_chain
        from shared_standard import validate_contradiction
    cp=load_json(CONTROL_PLANE_STATE_PATH); sm=load_json(STATE_MACHINE_PATH); hr=load_json(HANDOFF_REGISTRY_PATH); registry=load_json(REGISTRY_PATH); queue=load_json(QUEUE_PATH); claims=load_json(CLAIMS_PATH); contributions=load_json(CONTRIBUTION_PATH); contradictions=load_json(CONTRADICTION_PATH)
    ...
    return {"mission_count":len(cp["missions"]),"handoff_count":len(hr["items"]),"queue_count":len(queue["items"]),"claim_count":len(claims["claims"]),"contribution_count":len(contributions["items"]),"contradiction_count":len(contradictions["items"]),"event_count":len(events),"event_head":last_hash,"status":cp["status"]}
...
def main()->int:
    ...
    print(f"MISSION_ID={s['mission_id']}"); print(f"AGENT_ROLE={s['mission_identity']['agent_role']}"); print(f"ENGINEERING_STATUS={s['current_state']['ENGINEERING_STATUS']}"); print(f"SCIENTIFIC_LIMITATION_RESOLUTION={s['current_state']['SCIENTIFIC_LIMITATION_RESOLUTION']}"); print(f"PROSPECTIVE_PREDICTIVE_VALIDITY={s['current_state']['PROSPECTIVE_PREDICTIVE_VALIDITY']}"); print(f"CONTROL_PLANE={cp['status']}"); print(f"CONTROL_PLANE_MISSIONS={cp['mission_count']}"); print(f"CONTROL_PLANE_HANDOFFS={cp['handoff_count']}"); print(f"CONTROL_PLANE_CLAIMS={cp['claim_count']}"); print(f"CONTROL_PLANE_CONTRIBUTIONS={cp['contribution_count']}"); print(f"CONTROL_PLANE_CONTRADICTIONS={cp['contradiction_count']}"); print(f"CONTROL_PLANE_EVENTS={cp['event_count']}"); print(f"EVENT_CHAIN_HEAD={cp['event_head']}"); print("SHARED_STANDARD=LOADED_AND_EXECUTABLE"); print("RESPONSE_COUPLING=ACTIVE_FRONTIER"); print("MISSION_STATE=VALID"); return 0
