...
SCIENTIFIC_NORMATIVE_REGISTRY_PATH=MISSION_DIR/"SCIENTIFIC_NORMATIVE_REGISTRY.json"; SCIENTIFIC_NORMATIVE_DOCUMENT_PATH=MISSION_DIR.parent/"docs"/"scientific"/"SCIENTIFIC_NORMATIVE_STANDARD_001.md"
...
def validate_repository_layout():
    for p in (MEMORY_PATH,RECONCILIATION_PATH,CURRENT_RECONCILIATION_PATH,CONTROL_PLANE_STATE_PATH,STATE_MACHINE_PATH,HANDOFF_REGISTRY_PATH,REGISTRY_PATH,QUEUE_PATH,EVENT_LOG_PATH,CLAIMS_PATH,CONTRIBUTION_PATH,CONTRADICTION_PATH,LIFECYCLE_PATH,CONSTITUTION_PATH,SHARED_STANDARD_PATH,ROMAN_CONTRACT_PATH,ROMAN_STATE_PATH,SCIENTIFIC_NORMATIVE_REGISTRY_PATH,SCIENTIFIC_NORMATIVE_DOCUMENT_PATH):
        if not p.exists(): raise FileNotFoundError(f"Missing mission persistence artifact: {p}")

def validate_scientific_normative_standard():
    registry=load_json(SCIENTIFIC_NORMATIVE_REGISTRY_PATH)
    canonical=registry.get("canonical_document",{})
    if registry.get("status")!="ACTIVE": raise ValueError("Scientific normative registry is not ACTIVE")
    if canonical.get("document_id")!="SCIENTIFIC-NORMATIVE-STANDARD-001": raise ValueError("Unexpected canonical scientific normative document")
    if canonical.get("path")!="docs/scientific/SCIENTIFIC_NORMATIVE_STANDARD_001.md": raise ValueError("Scientific normative document path mismatch")
    if canonical.get("version")!="1.0.0" or canonical.get("status")!="ACTIVE" or canonical.get("mandatory_reading") is not True: raise ValueError("Scientific normative document is not active mandatory reading")
    text=SCIENTIFIC_NORMATIVE_DOCUMENT_PATH.read_text(encoding="utf-8")
    required_markers=("## 2. Epistemic separation","## 5. Scientific impact and blast radius","## 7. Temporal and point-in-time integrity","## 9. Predictive methodology","## 10. Causal, predictive and operational separation","## 14. Bibliographic integration","## 16. Fixed point","## 18. Mandatory AI behavior")
    if any(marker not in text for marker in required_markers): raise ValueError("Canonical scientific normative document is incomplete")
