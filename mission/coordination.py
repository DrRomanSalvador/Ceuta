"""Constitutional COORDINATOR/ESPEJO runtime, extending the existing control plane."""
...
STATE_PATH=Path(__file__).with_name("COORDINATION_STATE.json"); EVENT_PATH=Path(__file__).with_name("MISSION_EVENT_LOG.jsonl")
...
def initial(): return {"schema_version":"1.2.0","state_version":0,"updated_at":now(),"agents":{},"missions":{},"tasks":{},"priorities":{},"checkpoints":{},"commands":{},"interventions":[],"mirrors":{},"standby":{},"recovery":[],"conflicts":[],"handoffs":[],"pending_reviews":[],"deferred_hypotheses":{}}
@contextmanager
def locked(path):
 p=path.with_suffix(path.suffix+".lock"); p.parent.mkdir(parents=True,exist_ok=True)
 with p.open("a+") as h:
...
class Command: order_id:str; issuer:str; target:str; mission:str; reason:str; authority_basis:str; issued_at:str; state:str; expected_effect:str; acknowledgement:str|None; execution_result:str|None; command_type:str; expected_version:int; reversible:bool; provenance:dict[str,str]
@dataclass(frozen=True)
class MirrorInvocation: mirror_id:str; mirror_of:str; mission:str; target_agent:str; task:str; problem:str; capability_required:str; limits:tuple[str,...]; priority:str; authority:str; expected_result:str; action:str; functional_role:str; state:str; mission_owner:str; subtask_owner:str; provenance:dict[str,str]
@dataclass(frozen=True)
class DeferredHypothesis: hypothesis_id:str; author:str; timestamp:str; context:str; reasoning:str; alternative_proposed:str; suspension_reason:str
class CoordinationStore:
 def __init__(self,state_path=STATE_PATH,event_path=EVENT_PATH): self.state_path=Path(state_path); self.event_path=Path(event_path)
 def load(self):
  if not self.state_path.exists(): return initial()
  d=json.loads(self.state_path.read_text(encoding="utf-8")); self.validate(d); return d
 @staticmethod
 def validate(d):
  missing=set(initial())-set(d)
  if missing: raise CoordinationError(f"coordination state missing fields: {sorted(missing)}")
  if not isinstance(d.get("state_version"),int) or d["state_version"]<0: raise CoordinationError("invalid coordination state_version")
  if d.get("schema_version") not in {"1.0.0","1.1.0","1.2.0"}: raise CoordinationError("unsupported coordination schema")
 def write(self,d):
  self.validate(d); self.state_path.parent.mkdir(parents=True,exist_ok=True); fd,tmp=tempfile.mkstemp(prefix="coord-",suffix=".json",dir=self.state_path.parent)
  try:
   with os.fdopen(fd,"w",encoding="utf-8") as h: json.dump(d,h,sort_keys=True,indent=2,ensure_ascii=False); h.flush(); os.fsync(h.fileno())
...
class Coordinator:
 ...
 def register_agent(self,agent_id,mission_id,*,state="ACTIVE",progress=True,expected_version=None):
  def m(s): s["agents"][agent_id]={"agent_id":agent_id,"mission_id":mission_id,"state":state,"progress":bool(progress),"last_heartbeat":now(),"last_checkpoint":None,"last_change":now()}
 ...
 def update_observation(self,agent_id,*,state,progress,last_checkpoint=None,expected_version=None):
  ...
  def m(s):
   ...
   s["agents"][agent_id].update({"state":state,"progress":bool(progress),"last_heartbeat":now(),"last_change":now()})
 ...
 def _conflict(self,mission,reason,target,expected_version=None):
  def m(s): s["conflicts"].append({"type":"COORDINATION_CONFLICT","mission":mission,"target":target,"reason":reason,"actor":self.instance_id,"timestamp":now()})
  return self.store.mutate(actor=self.instance_id,event_type="COORDINATION_CONFLICT",mission_id=mission,expected_version=expected_version,mutation=m)
 def issue(self,*,command_type,target,mission,reason,authority_basis,expected_effect,expected_version=None,reversible=True,order_id=None):
  ...
  if authority_basis not in {Authority.COORDINATOR.value,Authority.HUMAN.value}: raise CommandConflict("invalid operational authority")
  if command_type in {CommandType.REDIRECT,CommandType.ABANDON} and authority_basis!=Authority.HUMAN.value: raise CommandConflict(f"{command_type.value} requires human authority")
  s=self.store.load(); v=s["state_version"]
  if expected_version is not None and expected_version!=v: raise StaleStateError(f"expected coordination version {expected_version}, observed {v}")
  oid=order_id or f"ORD-{uuid.uuid4().hex}"; old=s["commands"].get(oid)
  if old:
   if all(old.get(k)==x for k,x in (("target",target),("mission",mission),("command_type",command_type.value),("expected_effect",expected_effect))): return s
   self._conflict(mission,f"command replay/collision for {oid}",target); raise CommandConflict(f"command replay/collision for {oid}")
  contradictory={(CommandType.PAUSE.value,CommandType.RESUME.value),(CommandType.STANDBY.value,CommandType.CONTINUE.value),(CommandType.CONTINUE.value,CommandType.STANDBY.value),(CommandType.RESUME.value,CommandType.PAUSE.value)}
  for c in s["commands"].values():
   if c.get("mission")==mission and c.get("target")==target and (c.get("command_type"),command_type.value) in contradictory and c.get("state") in {CommandState.ISSUED.value,CommandState.ACKNOWLEDGED.value,CommandState.EXECUTING.value}:
    self._conflict(mission,f"contradictory command {c['order_id']}",target); raise CommandConflict("COORDINATION_CONFLICT: contradictory active command")
  t=now(); prov={"DISCOVERED_BY":self.instance_id,"PROPOSED_BY":self.instance_id,"IMPLEMENTED_BY":self.instance_id,"REVIEWED_BY":"PENDING","VALIDATED_BY":"PENDING","AUTHORIZED_BY":authority_basis,"COMMAND_ISSUER":self.instance_id}
  c=Command(oid,self.instance_id,target,mission,reason,authority_basis,t,CommandState.ISSUED.value,expected_effect,None,None,command_type.value,v,reversible,prov)
  def m(n): n["commands"][oid]=asdict(c); n["interventions"].append({"order_id":oid,"issuer":self.instance_id,"type":command_type.value,"target":target,"mission":mission,"timestamp":t,"provenance":prov})
 ...
 def checkpoint_then_pause(self,*,target,mission,checkpoint_id,expected_version=None):
  def m(s): s["checkpoints"][checkpoint_id]={"checkpoint_id":checkpoint_id,"mission":mission,"target":target,"created_at":now(),"state":"PERSISTED"}
 ...
 def pause(self,*,target,mission,checkpoint_id=None,expected_version=None):
  if checkpoint_id and checkpoint_id in self.store.load()["checkpoints"]: return self.issue(command_type=CommandType.PAUSE,target=target,mission=mission,reason="pause after persisted checkpoint",authority_basis=Authority.COORDINATOR.value,expected_effect="paused and recoverable",expected_version=expected_version)
  def m(s): s["recovery"].append({"type":"PAUSE_WITHOUT_CHECKPOINT_RISK","target":target,"mission":mission,"timestamp":now(),"recovery_required":True})
  s=self.store.mutate(actor=self.instance_id,event_type="PAUSE_WITHOUT_CHECKPOINT_RISK",mission_id=mission,expected_version=expected_version,mutation=m)
  return self.issue(command_type=CommandType.PAUSE,target=target,mission=mission,reason="checkpoint unavailable; explicit recovery risk recorded",authority_basis=Authority.COORDINATOR.value,expected_effect="pause with recovery risk",expected_version=s["state_version"])
 def standby(self,*,target,mission,checkpoint_id,expected_version=None):
  ...
  s=self.issue(command_type=CommandType.STANDBY,target=target,mission=mission,reason="temporary operational standby",authority_basis=Authority.COORDINATOR.value,expected_effect="identity and recoverability preserved",expected_version=expected_version)
  def m(n): n["standby"][target]={"target":target,"mission":mission,"checkpoint_id":checkpoint_id,"state":"STANDBY","entered_at":now()}
  return self.store.mutate(actor=self.instance_id,event_type="COORDINATOR_STANDBY",mission_id=mission,expected_version=s["state_version"],mutation=m)
 def resume(self,*,target,mission,expected_version=None):
  s=self.issue(command_type=CommandType.RESUME,target=target,mission=mission,reason="resume from standby",authority_basis=Authority.COORDINATOR.value,expected_effect="execution resumes from persisted state",expected_version=expected_version)
  def m(n): n["standby"].pop(target,None)
  return self.store.mutate(actor=self.instance_id,event_type="COORDINATOR_RESUME",mission_id=mission,expected_version=s["state_version"],mutation=m)
 def reprioritize(self,*,mission,task,priority,expected_version=None):
  s=self.issue(command_type=CommandType.REPRIORITIZE,target=task,mission=mission,reason="priority update",authority_basis=Authority.COORDINATOR.value,expected_effect=priority,expected_version=expected_version)
  def m(n): n["priorities"][task]={"mission":mission,"priority":priority,"updated_by":self.instance_id,"updated_at":now()}
  return self.store.mutate(actor=self.instance_id,event_type="COORDINATOR_REPRIORITIZE",mission_id=mission,expected_version=s["state_version"],mutation=m)
 def handoff(self,*,target,mission,handoff_id,expected_version=None):
  s=self.issue(command_type=CommandType.HANDOFF,target=target,mission=mission,reason="formal operational handoff",authority_basis=Authority.COORDINATOR.value,expected_effect="handoff persisted",expected_version=expected_version)
  def m(n): n["handoffs"].append({"handoff_id":handoff_id,"mission":mission,"target":target,"issued_by":self.instance_id,"timestamp":now(),"status":"PENDING"})
  return self.store.mutate(actor=self.instance_id,event_type="COORDINATOR_HANDOFF",mission_id=mission,expected_version=s["state_version"],mutation=m)
 def recover(self,*,target,mission,reason,expected_version=None):
  s=self.issue(command_type=CommandType.RECOVER,target=target,mission=mission,reason=reason,authority_basis=Authority.COORDINATOR.value,expected_effect="recover from persisted checkpoint/event state",expected_version=expected_version)
  def m(n): n["recovery"].append({"type":"RECOVER","target":target,"mission":mission,"reason":reason,"timestamp":now(),"status":"REQUESTED"})
  return self.store.mutate(actor=self.instance_id,event_type="COORDINATOR_RECOVERY",mission_id=mission,expected_version=s["state_version"],mutation=m)
...
class Mirror:
 ...
 def invoke(self,*,mirror_of,mission,target_agent,task,problem,capability_required,limits,priority,authority,expected_result,action,functional_role,mission_owner):
  ...
  self.preflight(target_agent=target_agent,mission=mission,task=task)
 ...
 def complete(self,mirror_id,*,result,validated,expected_version=None):
  ...
  def m(s):
   ...
   s["handoffs"].append({"mirror_id":mirror_id,"mission_owner":inv["mission_owner"],"subtask_owner":self.instance_id,"result":result,"timestamp":now(),"control_returned":True,"provenance":inv["provenance"]})
