...
def validate_chain(events: list[dict[str, Any]]) -> str:
    previous=GENESIS_HASH; seen:set[str]=set()
...
def append_event(path:Path,event:dict[str,Any])->str:
    with _locked(path):
        events=load_jsonl(path); last=validate_chain(events)
        if event["previous_hash"]!=last: raise ValueError("Event does not extend current chain; caller must reload and retry")
        validate_chain(events+[event]); path.parent.mkdir(parents=True,exist_ok=True)
        with path.open("a",encoding="utf-8") as handle:
            handle.write(json.dumps(event,sort_keys=True,separators=(",",":"),ensure_ascii=False)+"\n"); handle.flush()
        return event["hash"]


def append_payload(path:Path,*,event_type:str,mission_id:str,actor:str,timestamp:str,payload:dict[str,Any])->dict[str,Any]:
    """Create and append a transition event from the current chain head atomically."""
    with _locked(path):
        events=load_jsonl(path); previous=validate_chain(events); event_id=f"EV-{len(events)+1:04d}"
        if any(e["event_id"]==event_id for e in events): raise ValueError("Event ID collision; chain must be reconciled")
        event=make_event(event_id=event_id,event_type=event_type,mission_id=mission_id,actor=actor,timestamp=timestamp,payload=payload,previous_hash=previous)
        validate_chain(events+[event]); path.parent.mkdir(parents=True,exist_ok=True)
        with path.open("a",encoding="utf-8") as handle:
            handle.write(json.dumps(event,sort_keys=True,separators=(",",":"),ensure_ascii=False)+"\n"); handle.flush()
        return event
