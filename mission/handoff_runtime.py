...
def transition(path: Path, event_log: Path, handoff: dict[str,Any], *, target_status: HandoffState, actor: str, timestamp: str, evidence: list[str]) -> dict[str,Any]:
    ...
    legal={
        HandoffState.CREATED:{HandoffState.VALIDATION_PENDING,HandoffState.READY,HandoffState.REJECTED},
        HandoffState.VALIDATION_PENDING:{HandoffState.READY,HandoffState.REJECTED},
        HandoffState.READY:{HandoffState.ACCEPTED,HandoffState.REJECTED},
        HandoffState.ACCEPTED:{HandoffState.IMPLEMENTING,HandoffState.REJECTED},
        HandoffState.IMPLEMENTING:{HandoffState.IMPLEMENTED,HandoffState.REJECTED},
        HandoffState.IMPLEMENTED:{HandoffState.VERIFIED,HandoffState.REJECTED},
        HandoffState.VERIFIED:{HandoffState.INTEGRATED,HandoffState.SUPERSEDED},
        HandoffState.INTEGRATED:{HandoffState.SUPERSEDED},
        HandoffState.REJECTED:{HandoffState.SUPERSEDED},
        HandoffState.SUPERSEDED:set(),
    }
    ...
    with _locked(path):
        data=_load(path); existing=data["handoffs"].get(handoff["handoff_id"],dict(handoff))
        ...
        if target_status is HandoffState.INTEGRATED:
            record["integrated_at"]=timestamp; record["verification_state"]=HandoffState.VERIFIED.value
