...
Lifecycle mutations can optionally be backed by the canonical append-only event
log.  The JSON ledger remains a materialized projection; when event backing is
requested, the event is written first and its identity is persisted with the
projection record so reconciliation can detect projection lag.
"""
...
def _event(path: Path | None, *, event_type: str, mission_id: str, actor: str, payload: dict[str, Any], timestamp: str | None) -> dict[str, Any] | None:
    if path is None:
        return None
    return append_payload(path, event_type=event_type, mission_id=mission_id, actor=actor, timestamp=timestamp or __import__("datetime").datetime.now(__import__("datetime").timezone.utc).isoformat(), payload=payload)
