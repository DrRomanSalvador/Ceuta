...
def admit(path: Path, contract: dict[str, Any], *, authorized_by: str, evidence: list[str], event_log: Path, timestamp: str) -> dict[str, Any]:
    ...
    if not (mission_id.startswith("MISSION-") or mission_id == "ROMAN"):
        raise ValueError("Operational mission IDs must use MISSION-* or canonical ROMAN namespace")
...
def retire(path: Path, *, mission_id: str, owner: str, authorized_by: str, evidence: list[str], reason: str, event_log: Path, timestamp: str) -> dict[str, Any]:
    ...
    record["mutation_event_id"] = event["event_id"]
    data["retirements"].append(record)
    _write(path, data)
    return record
...
def record_conflict(path: Path, *, conflict_id: str, claim_a: str, claim_b: str, category: str, owner: str, evidence_a: list[str], evidence_b: list[str], event_log: Path, actor: str, timestamp: str) -> dict[str, Any]:
    ...
    if not evidence_a or not evidence_b:
        raise ValueError("Conflict requires evidence for both claims")
    data = load_ledger(path)
    ...
    event = _event(event_log, event_type="MISSION_CONTRADICTION", mission_id=owner, actor=actor, payload=record, timestamp=timestamp)
    record["mutation_event_id"] = event["event_id"]
    data["conflicts"].append(record)
    _write(path, data)
    return record
