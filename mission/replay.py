...
def apply_event(state: ReplayState, event: dict[str, Any]) -> None:
    ...
    if kind == "CONTROL_PLANE_GENESIS": pass
        ...
        mission_id=event["mission_id"]; source=payload["from"]; target=payload["to"]
        ...
        handoff_id=payload["handoff_id"]; state.handoffs[handoff_id]={**payload,"event_id":event["event_id"]}
        ...
        mission_id=event["mission_id"]; expected=payload["expected_revision"]; new_revision=payload["new_revision"]
