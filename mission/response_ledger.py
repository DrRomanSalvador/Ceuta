...
def _load(path:Path)->dict[str,Any]:
    if not path.exists(): return {"schema_version":"1.1.0","records":[]}
...
def validate_response_record(record:dict[str,Any])->None:
    ...
    if record["execution_status"]=="NOT_EXECUTED" and record["implementation_failure"] in (None, ""): raise ValueError("NOT_EXECUTED response requires implementation_failure")
    ...
    if record["causal_status"]=="IDENTIFICATION_SUPPORTED":
        if not record["causal_method"]: raise ValueError("Causal effectiveness requires a declared causal method")
