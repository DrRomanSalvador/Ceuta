...
class LifecycleGovernanceTests(unittest.TestCase):
    def contract(self, mission_id="MISSION-99"):
        return {
            ...
            "dependencies": [],
            "handoff_contract": "x",
            "validation_contract": "x",
            "recovery_policy": "x",
            "retirement_conditions": ["x"],
    ...
    def test_roman_admission_is_explicitly_runtime_pending(self):
        with tempfile.TemporaryDirectory() as tmp:
            path, events = self._paths(tmp)
            admit(path, self.contract("ROMAN"), authorized_by="MISSION-01", evidence=["main:MISSION_REGISTRY", "main:ROMAN_MISSION_STATE"], event_log=events, timestamp="2026-09-16T12:00:00Z")
    ...
    def test_retirement_is_persistent_and_authorized(self):
        with tempfile.TemporaryDirectory() as tmp:
            path, events = self._paths(tmp)
            record = retire(path, mission_id="MISSION-99", owner="MISSION-99", authorized_by="HUMAN_AUTHORITY", evidence=["e"], reason="superseded", event_log=events, timestamp="2026-09-16T12:00:00Z")
            self.assertEqual(record["status"], "RETIRED")
            self.assertTrue(record["mutation_event_id"])

    def test_recovery_is_persistent(self):
        with tempfile.TemporaryDirectory() as tmp:
            path, events = self._paths(tmp)
            record = recover(path, mission_id="MISSION-99", trigger="checkpoint-loss", evidence=["event-chain"], restored_state="ACTIVE", event_log=events, actor="SYSTEM", timestamp="2026-09-16T12:00:00Z")
    ...
    def test_conflict_preserves_both_claims(self):
        with tempfile.TemporaryDirectory() as tmp:
            path, events = self._paths(tmp)
            record = record_conflict(path, conflict_id="C1", claim_a="A", claim_b="B", category="factual", owner="MISSION-02", evidence_a=["ea"], evidence_b=["eb"], event_log=events, actor="MISSION-02", timestamp="2026-09-16T12:00:00Z")
            self.assertEqual(record["status"], "OPEN")
            self.assertEqual(len(load_ledger(path)["conflicts"]), 1)

    def test_event_lineage_covers_every_lifecycle_mutation(self):
        with tempfile.TemporaryDirectory() as tmp:
            path, events = self._paths(tmp)
            ...
            recovery = recover(path, mission_id="MISSION-99", trigger="checkpoint-loss", evidence=["event-chain"], restored_state="ACTIVE", event_log=events, actor="SYSTEM", timestamp="2026-09-16T12:02:00Z")
