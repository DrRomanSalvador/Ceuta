...
class ReplayTests(unittest.TestCase):
    ...
    def test_replay_reconstructs_response_coupling(self):
        with tempfile.TemporaryDirectory() as tmp:
            path=Path(tmp)/"events.jsonl"
            append_payload(path,event_type="CONTROL_PLANE_GENESIS",mission_id="MISSION-01",actor="MISSION-01",timestamp="2026-09-16T00:00:00Z",payload={})
            record={"mission_id":"MISSION-01","response_id":"RESP-1","warning_presence":"PRESENT","warning_or_prediction_identity":"PRED-1","decision_identity":"DEC-1","decision_time":"2026-09-16T12:00:00Z","action_identity":"ACT-1","execution_time":"2026-09-16T12:10:00Z","responsible_actor":"ORG-1","response_eligibility":{"eligible":True},"intended_mechanism":"declared","response_delay":600,"intervention_exposure_intensity":{"level":1},"implementation_failure":None,"resource_capacity_constraints":[],"outcome_ascertainment_identity":None,"response_horizon":"PT24H","counterfactual_causal_status":"INSUFFICIENT","execution_status":"EXECUTED","causal_status":"IDENTIFICATION_INSUFFICIENT"}
    ...
    def test_replay_reconstructs_materialized_state_cas(self):
        with tempfile.TemporaryDirectory() as tmp:
            path=Path(tmp)/"events.jsonl"
            append_payload(path,event_type="MATERIALIZED_STATE_CAS",mission_id="ROMAN",actor="MISSION-01",timestamp="2026-09-16T12:00:00Z",payload={"expected_revision":0,"new_revision":1,"projection":{"status":"ACTIVE"}})
            append_payload(path,event_type="MATERIALIZED_STATE_CAS",mission_id="ROMAN",actor="MISSION-01",timestamp="2026-09-16T12:01:00Z",payload={"expected_revision":1,"new_revision":2,"projection":{"status":"BLOCKED"}})
            state=replay(path)
            self.assertEqual(state.materialized_state["ROMAN"]["revision"],2)
            self.assertEqual(state.materialized_state["ROMAN"]["projection"]["status"],"BLOCKED")

    def test_replay_rejects_materialized_state_revision_gap(self):
        with tempfile.TemporaryDirectory() as tmp:
            path=Path(tmp)/"events.jsonl"
            append_payload(path,event_type="MATERIALIZED_STATE_CAS",mission_id="ROMAN",actor="MISSION-01",timestamp="2026-09-16T12:00:00Z",payload={"expected_revision":1,"new_revision":2,"projection":{}})
    ...
    def test_current_persistent_event_stream_replays_to_admitted_roman(self):
        ...
        state=replay(root/"mission"/"MISSION_EVENT_LOG.jsonl")
        roman=json.loads((root/"mission"/"ROMAN_MISSION_STATE.json").read_text(encoding="utf-8"))
        lifecycle=json.loads((root/"mission"/"MISSION_LIFECYCLE_LEDGER.json").read_text(encoding="utf-8"))
