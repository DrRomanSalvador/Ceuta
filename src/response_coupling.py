...
class ResponseBinding:
    ...
    def validate(self) -> None:
        ...
        if not self.prediction_identity: raise ValueError("prediction_identity is required")
        if not self.responsible_actor: raise ValueError("responsible_actor is required")
        if self.execution_status not in EXECUTION_STATES: raise ValueError("Unknown execution status")
        if self.causal_status not in CAUSAL_STATUSES: raise ValueError("Unknown causal status")
        if self.counterfactual_causal_status not in COUNTERFACTUAL_STATUSES: raise ValueError("Unknown counterfactual status")
        if self.decision_identity and not self.decision_time: raise ValueError("decision_time is required when decision_identity is supplied")
        if self.action_identity and not self.execution_time: raise ValueError("execution_time is required when action_identity is supplied")
        if self.execution_time and not self.action_identity: raise ValueError("action_identity is required when execution_time is supplied")
        ...
        if self.causal_status == "IDENTIFICATION_SUPPORTED":
            ...
            if not self.response_horizon: raise ValueError("Causal effectiveness requires a predeclared response horizon")
...
class ResponseCouplingSink:
    def __init__(self, writer: Callable[..., dict[str, Any]], mission_id: str):
        if not callable(writer): raise TypeError("writer must be callable")
        if not mission_id: raise ValueError("mission_id is required")
        self._writer=writer; self.mission_id=mission_id
    ...
    def record(self, alert: Any, binding: ResponseBinding, *, actor: str, timestamp: str) -> dict[str, Any]:
        ...
        if not timestamp: raise ValueError("timestamp is required")
