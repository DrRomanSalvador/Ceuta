...
class ScientificCapabilityTests(unittest.TestCase):
    ...
    def observation(self, offset_days: int, knowledge_delay: int = 0, vintage: str = "v1"):
        ...
        return ObservationRecord(observation_id=f"O-{offset_days}-{vintage}", phenomenon_id="P-1", value=float(offset_days), event_time=observed, observed_at=observed, publication_time=knowledge, revision_time=None, knowledge_time=knowledge, vintage_id=vintage, source_id="SOURCE-1")
    ...
    def test_dynamic_denominator_requires_population_at_risk(self):
        denominator = DynamicDenominator(denominator_id="D-1", population_definition="population at risk", population_at_risk=1000, exposed_population=500, observed_population=800, period_start=self.t0, period_end=self.t0 + timedelta(days=1), geography="CEUTA", coverage_fraction=0.8, definition_version="1")
    ...
    def test_observation_process_is_explicit_and_invertible_only_when_identified(self):
        process = ObservationProcess(process_id="OBS-1", phenomenon_id="P-1", detection_probability=0.5, reporting_fraction=0.8, coverage_fraction=0.5)
    ...
    def test_non_identifiable_state_requires_equivalent_alternative(self):
        with self.assertRaises(ValueError):
            IdentifiabilityAssessment(quantity_id="STATE-1", status=EpistemicIdentifiability.NOT_IDENTIFIABLE, uncertainty_description="insufficient observations")

    def test_identifiability_preserves_resolving_data(self):
        assessment = IdentifiabilityAssessment(quantity_id="STATE-1", status=EpistemicIdentifiability.PARTIALLY_IDENTIFIED, informed_by=("Y-1",), observationally_equivalent_alternatives=("A", "B"), uncertainty_description="two mechanisms remain observationally equivalent", resolving_data=("SOURCE-2",))
    ...
    def test_normal_crps_is_non_negative(self):
        ...
        forecasts = [ProbabilisticForecast(mean=0.0, std=1.0, observed=0.0), ProbabilisticForecast(mean=1.0, std=2.0, observed=1.5)]
    ...
    def test_expected_utility_is_exposed_as_action_comparison(self):
        no_action, action = expected_binary_loss(0.8, action_cost=1.0, false_positive_cost=2.0, false_negative_cost=10.0)
