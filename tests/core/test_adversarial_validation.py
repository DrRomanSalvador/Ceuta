...
def test_valid_candidate_passes_all_six_expert_gates() -> None:
    ...
    report = run_adversarial_validation(candidate, now=datetime(2026, 9, 9, 10, 0, tzinfo=UTC))
    ...
    assert {result.domain for result in report.expert_results} == set(ExpertDomain)
    assert report.findings == ()


def test_missing_provenance_prevents_production() -> None:
    report = run_adversarial_validation(_valid_candidate(provenance_complete=False))
...
def test_dependent_sources_are_not_treated_as_independent() -> None:
    report = run_adversarial_validation(_valid_candidate(source_independence_assessed=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_unresolved_contradictions_are_detected() -> None:
    report = run_adversarial_validation(_valid_candidate(contradiction_handling=False))
...
def test_missing_temporal_validation_blocks_operational_maturity() -> None:
    report = run_adversarial_validation(_valid_candidate(temporal_validation=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_missing_probability_calibration_is_critical() -> None:
    report = run_adversarial_validation(_valid_candidate(calibration_assessed=False))
...
def test_missing_probability_discrimination_is_major() -> None:
    report = run_adversarial_validation(_valid_candidate(discrimination_assessed=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_probability_without_uncertainty_is_not_production_ready() -> None:
    report = run_adversarial_validation(_valid_candidate(uncertainty_quantified=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_missing_outcome_definition_is_detected() -> None:
    report = run_adversarial_validation(_valid_candidate(event_definition=None))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_missing_prediction_horizon_is_detected() -> None:
    report = run_adversarial_validation(_valid_candidate(prediction_horizon_minutes=None))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_negative_prediction_horizon_is_detected() -> None:
    report = run_adversarial_validation(_valid_candidate(prediction_horizon_minutes=-1.0))
    assert report.decision is ValidationDecision.SHADOW_ONLY
    assert FindingCode.HORIZON_UNDEFINED in report.unresolved_major_failures


def test_false_positive_cost_must_be_assessed() -> None:
    report = run_adversarial_validation(_valid_candidate(false_positive_cost_assessed=False))
...
def test_false_negative_cost_must_be_assessed() -> None:
    report = run_adversarial_validation(_valid_candidate(false_negative_cost_assessed=False))
...
def test_lead_time_must_be_measured() -> None:
    report = run_adversarial_validation(_valid_candidate(lead_time_assessed=False))
...
def test_operational_utility_must_be_evaluated() -> None:
    report = run_adversarial_validation(_valid_candidate(operational_utility_assessed=False))
...
def test_direct_automated_operational_action_is_critical() -> None:
    report = run_adversarial_validation(_valid_candidate(automated_operational_action=True))
    assert report.decision is ValidationDecision.BLOCK
...
def test_identity_based_prediction_is_critical() -> None:
    report = run_adversarial_validation(_valid_candidate(uses_identity_attributes=True))
    assert report.decision is ValidationDecision.BLOCK
...
def test_individual_dangerousness_prediction_is_critical() -> None:
    report = run_adversarial_validation(_valid_candidate(target_level=TargetLevel.INDIVIDUAL, predicts_individual_dangerousness=True))
    assert report.decision is ValidationDecision.BLOCK
...
def test_group_dangerousness_prediction_is_critical() -> None:
    report = run_adversarial_validation(_valid_candidate(target_level=TargetLevel.GROUP, predicts_group_dangerousness=True))
    assert report.decision is ValidationDecision.BLOCK
...
def test_individual_target_is_not_accepted_by_geospatial_gate() -> None:
    report = run_adversarial_validation(_valid_candidate(target_level=TargetLevel.INDIVIDUAL))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_group_target_without_dangerousness_prediction_can_be_reviewed() -> None:
    report = run_adversarial_validation(_valid_candidate(target_level=TargetLevel.GROUP, predicts_group_dangerousness=False))
    assert report.decision is ValidationDecision.PASS


def test_missing_privacy_review_prevents_production() -> None:
    report = run_adversarial_validation(_valid_candidate(privacy_reviewed=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_missing_cybersecurity_review_prevents_production() -> None:
    report = run_adversarial_validation(_valid_candidate(cybersecurity_reviewed=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_missing_reproducibility_prevents_production() -> None:
    report = run_adversarial_validation(_valid_candidate(reproducibility_verified=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_no_scientific_references_prevents_production() -> None:
    report = run_adversarial_validation(_valid_candidate(scientific_references=()))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_prospective_validation_claim_without_prospective_dataset_is_critical() -> None:
    report = run_adversarial_validation(_valid_candidate(validation_dataset=_dataset(prospective_evaluation=False)))
    assert report.decision is ValidationDecision.BLOCK
...
def test_external_validation_claim_requires_external_dataset() -> None:
    report = run_adversarial_validation(_valid_candidate(evidence_maturity=EvidenceMaturity.EXTERNALLY_VALIDATED, validation_dataset=_dataset(external_dataset=False)))
    assert report.decision is ValidationDecision.BLOCK
    assert FindingCode.INSUFFICIENT_EVIDENCE in report.mandatory_failures


def test_temporal_validation_claim_without_temporal_validation_is_critical() -> None:
    report = run_adversarial_validation(_valid_candidate(evidence_maturity=EvidenceMaturity.TEMPORALLY_VALIDATED, temporal_validation=False))
    assert report.decision is ValidationDecision.BLOCK
...
def test_missing_validation_dataset_is_major_not_silent() -> None:
    report = run_adversarial_validation(_valid_candidate(validation_dataset=None))
    assert report.decision is ValidationDecision.SHADOW_ONLY
    assert FindingCode.INSUFFICIENT_EVIDENCE in report.unresolved_major_failures


def test_validation_dataset_with_no_positive_events_is_rejected() -> None:
    dataset = ValidationDataset(dataset_id="no-events", sample_size=1_000, positive_events=0, negative_events=1_000, temporal_split=True, external_dataset=True, prospective_evaluation=True, independent_test_set=True, missingness_assessed=True, subgroup_evaluation=True, drift_assessed=True)
    report = run_adversarial_validation(_valid_candidate(validation_dataset=dataset))
    assert report.decision is ValidationDecision.SHADOW_ONLY
    assert FindingCode.INSUFFICIENT_EVIDENCE in report.unresolved_major_failures
...
@pytest.mark.parametrize(
    ("field", "value"),
    [
        ("roc_auc", -0.1), ("roc_auc", 1.1), ("pr_auc", -0.1), ("pr_auc", 1.1),
        ("brier_score", -0.1), ("brier_score", 1.1), ("sensitivity", -0.1),
        ("sensitivity", 1.1), ("specificity", -0.1), ("specificity", 1.1),
        ("false_positive_rate", -0.1), ("false_positive_rate", 1.1),
        ("false_negative_rate", -0.1), ("false_negative_rate", 1.1),
    ],
)
def test_performance_bounds_are_enforced(field: str, value: float) -> None:
    report = run_adversarial_validation(_valid_candidate(performance=PredictivePerformance(**{field: value})))
    assert report.decision is ValidationDecision.BLOCK


def test_negative_lead_time_is_rejected() -> None:
    report = run_adversarial_validation(_valid_candidate(performance=PredictivePerformance(lead_time_minutes=-10.0)))
    assert report.decision is ValidationDecision.BLOCK


def test_nonfinite_performance_is_rejected() -> None:
    report = run_adversarial_validation(_valid_candidate(performance=PredictivePerformance(roc_auc=float("nan"))))
    assert report.decision is ValidationDecision.BLOCK


def test_missingness_assessment_is_required() -> None:
    report = run_adversarial_validation(_valid_candidate(missingness_assessed=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
    assert FindingCode.MISSINGNESS_UNASSESSED in report.unresolved_major_failures


def test_drift_assessment_is_required() -> None:
    report = run_adversarial_validation(_valid_candidate(drift_assessed=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
    assert FindingCode.DRIFT_UNASSESSED in report.unresolved_major_failures


def test_robustness_assessment_is_required() -> None:
    report = run_adversarial_validation(_valid_candidate(robustness_assessed=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_subgroup_robustness_assessment_is_required() -> None:
    report = run_adversarial_validation(_valid_candidate(subgroup_robustness_assessed=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
    assert FindingCode.SUBGROUP_ROBUSTNESS_UNASSESSED in report.unresolved_major_failures


def test_human_oversight_is_required_for_operational_use() -> None:
    report = run_adversarial_validation(_valid_candidate(human_oversight_implemented=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
...
def test_multiple_major_failures_are_preserved() -> None:
    report = run_adversarial_validation(_valid_candidate(provenance_complete=False, temporal_validation=False, uncertainty_quantified=False, privacy_reviewed=False, cybersecurity_reviewed=False))
    assert report.decision is ValidationDecision.SHADOW_ONLY
    expected = {FindingCode.MISSING_PROVENANCE, FindingCode.NO_TEMPORAL_VALIDATION, FindingCode.UNCERTAINTY_MISSING, FindingCode.PRIVACY_VIOLATION, FindingCode.SECURITY_VIOLATION}
...
def test_critical_failure_has_priority_over_major_failures() -> None:
    report = run_adversarial_validation(_valid_candidate(privacy_reviewed=False, calibration_assessed=False))
    assert report.decision is ValidationDecision.BLOCK
    assert FindingCode.UNCALIbrATED_PROBABILITY in report.mandatory_failures
    assert FindingCode.PRIVACY_VIOLATION in report.unresolved_major_failures
...
def test_summary_is_serializable_and_does_not_expose_private_content() -> None:
    report = run_adversarial_validation(_valid_candidate(provenance_complete=False, privacy_reviewed=False))
...
def test_report_timestamp_is_deterministic_when_supplied() -> None:
    ...
    report = run_adversarial_validation(_valid_candidate(), now=timestamp)
    assert report.generated_at == timestamp


def test_all_six_experts_are_always_executed() -> None:
    candidate = _valid_candidate(provenance_complete=False, automated_operational_action=True, spatial_validation=False, privacy_reviewed=False, calibration_assessed=False)
    report = run_adversarial_validation(candidate)
    domains = {result.domain for result in report.expert_results}
    assert domains == {ExpertDomain.STRATEGIC_INTELLIGENCE, ExpertDomain.OPERATIONAL_SECURITY, ExpertDomain.EPIDEMIOLOGY_PUBLIC_HEALTH, ExpertDomain.GEOSPATIAL_POPULATION, ExpertDomain.LAW_PRIVACY_CYBERSECURITY, ExpertDomain.STATISTICS_ML}
    assert report.decision is ValidationDecision.BLOCK


def test_a_candidate_cannot_pass_by_hiding_a_critical_problem_in_metadata() -> None:
    candidate = _valid_candidate(metadata={"identity_risk": "true", "dangerousness_model": "enabled", "automated_action": "enabled"})
    report = run_adversarial_validation(candidate)
    assert report.decision is ValidationDecision.PASS


def test_system_level_event_prediction_is_allowed_when_all_gates_pass() -> None:
    report = run_adversarial_validation(_valid_candidate(target_level=TargetLevel.EVENT, event_definition="Predefined public-order event"))
    assert report.decision is ValidationDecision.PASS


def test_population_level_health_prediction_is_allowed_when_validated() -> None:
    report = run_adversarial_validation(_valid_candidate(target_level=TargetLevel.POPULATION, event_definition="Population-level health-system overload event"))
    assert report.decision is ValidationDecision.PASS


def test_information_about_a_group_is_not_equivalent_to_group_dangerousness() -> None:
    report = run_adversarial_validation(_valid_candidate(target_level=TargetLevel.GROUP, predicts_group_dangerousness=False, event_definition="Aggregate population-flow change"))
    assert report.decision is ValidationDecision.PASS
...
def test_finding_severities_are_preserved() -> None:
    report = run_adversarial_validation(_valid_candidate(uses_identity_attributes=True, temporal_validation=False))
    critical_findings = [finding for finding in report.findings if finding.severity is FindingSeverity.CRITICAL]
    major_findings = [finding for finding in report.findings if finding.severity is FindingSeverity.MAJOR]
