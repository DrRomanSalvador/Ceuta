...
def _evidence(**kwargs):
    ...
    defaults = dict(
        ...
        value=1.0,
        unit=None,
        semantic_definition="test evidence",
        location=None,
        event_time=base,
        publication_time=base,
        ingestion_time=base + timedelta(hours=1),
        revision_time=None,
        detection_time=None,
...
def test_evidence_effective_validity_is_point_in_time_aware():
    ...
    assert evidence.is_valid_at(base + timedelta(hours=1)) is True
    assert evidence.is_valid_at(base + timedelta(hours=2)) is True
    assert evidence.is_valid_at(base + timedelta(hours=3)) is False
