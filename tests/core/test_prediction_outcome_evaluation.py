...
def _record(connection: sqlite3.Connection, **overrides: object) -> dict[str, object]:
    ...
    prediction=_prediction(); prediction["origin_time"]=(now-timedelta(hours=2)).isoformat(); prediction["available_at"]=(now-timedelta(hours=1,minutes=55)).isoformat()
    unsigned=dict(prediction); unsigned.pop("integrity_hash",None); prediction["integrity_hash"]=sha256(json.dumps(unsigned,sort_keys=True,separators=(",",":"),default=str).encode()).hexdigest()
    record_prediction(connection,prediction,decision_id="decision-1")
    args={"prediction_id":"prediction-outcome-1","decision_id":"decision-1","action_id":"option-1","outcome_id":"outcome-1","target":"target.binary","outcome_time":now,"observed":1,"provenance":("outcome:test",),**_ascertainment(now)}
...
def test_prediction_outcome_is_linked_and_evaluated():
    ...
    assert stored["action_id"]=="option-1" and stored["source_version"]=="2026.09" and stored["outcome_definition_version"]=="target.binary:v1"
...
def test_outcome_cannot_precede_prediction_target_time():
    ...
    with pytest.raises(ValueError,match="target time"):
        _record(connection,outcome_time=now-timedelta(hours=1))
...
def test_outcome_cannot_be_ascertained_before_prediction_is_available():
    ...
    with pytest.raises(ValueError,match="cannot precede prediction availability"):
        _record(connection,observation_time=now-timedelta(hours=3,minutes=1),availability_time=now-timedelta(hours=3),ascertainment_time=now)
