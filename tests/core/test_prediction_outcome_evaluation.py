...
def test_prediction_outcome_is_linked_and_evaluated():
    connection=sqlite3.connect(":memory:"); result=_record(connection); assert result["brier_error"]==pytest.approx(0.0625); assert result["log_loss_error"]==pytest.approx(-math.log(0.75)); stored=get_prediction_outcome(connection,"prediction-outcome-1"); assert stored["action_id"]=="option-1"; assert stored["source_version"]=="2026.09"; assert stored["outcome_definition_version"]=="target.binary:v1"

def test_outcome_cannot_precede_prediction_availability():
    connection=sqlite3.connect(":memory:"); with pytest.raises(ValueError,match="availability"): _record(connection,outcome_time=datetime(2026,9,15,6,1,tzinfo=timezone.utc))

def test_outcome_cannot_precede_prediction_target_time():
    connection=sqlite3.connect(":memory:"); now=datetime.now(timezone.utc).replace(microsecond=0); with pytest.raises(ValueError,match="target time"): _record(connection,outcome_time=now-timedelta(hours=1))

def test_outcome_requires_prediction_decision_alignment():
    connection=sqlite3.connect(":memory:"); with pytest.raises(ValueError,match="does not belong"): _record(connection,decision_id="decision-2")

def test_outcome_rejects_unordered_ascertainment_timestamps():
    connection=sqlite3.connect(":memory:"); now=datetime.now(timezone.utc).replace(microsecond=0); with pytest.raises(ValueError,match="must be ordered"): _record(connection,**{"observation_time":now,"availability_time":now-timedelta(seconds=1)})
...
def test_outcome_revision_creates_identity_distinction_and_collision_is_rejected():
    connection=sqlite3.connect(":memory:"); _record(connection); with pytest.raises(RuntimeError,match="identity collision"): _record(connection,revision_id="rev-2")

def test_outcome_cannot_be_ascertained_before_prediction_is_available():
    connection=sqlite3.connect(":memory:"); now=datetime.now(timezone.utc).replace(microsecond=0); with pytest.raises(ValueError,match="cannot precede prediction availability"): _record(connection,**{"observation_time":now-timedelta(hours=3,minutes=1),"availability_time":now-timedelta(hours=3),"ascertainment_time":now})
