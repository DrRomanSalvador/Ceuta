...
def test_product_replay_and_outcome_endpoints(tmp_path, monkeypatch):
    ...
    with TestClient(main.app) as client:
        ...
        replay = client.get("/scientific/predictions/prediction-1/replay", params={"as_of": now.isoformat()}, headers=headers)
        assert replay.status_code == 200
        assert replay.json()["point_in_time_eligible"] is True
        assert replay.json()["scientific_contract_valid"] is True
        early = client.get("/scientific/predictions/prediction-1/replay", params={"as_of": (origin - timedelta(seconds=1)).isoformat()}, headers=headers)
        assert early.status_code == 422

        outcome = client.post("/scientific/predictions/outcomes", headers=headers, json={"prediction_id": "prediction-1", "decision_id": "decision-1", "action_id": "option-1", "outcome_id": "outcome-1", "target": "risk", "outcome_time": now.isoformat(), "observed": 1, "provenance": ["test:outcome"]})
        assert outcome.status_code == 200
        assert outcome.json()["prediction_id"] == "prediction-1"
        assert outcome.json()["brier_error"] == pytest.approx(0.09)

        evaluation = client.get("/scientific/predictions/evaluation", params={"target": "risk", "horizon": "24h"}, headers=headers)
        assert evaluation.status_code == 200
        assert evaluation.json()["n"] == 1
        assert evaluation.json()["status"] == "descriptive_prospective_evaluation"
        assert evaluation.json()["protocol_id"] == "CEUTIA-SERPIENTE-PREDICTIVE-EVAL-1"
        assert evaluation.json()["prospective_validity_status"] == "NOT_ESTABLISHED"
        assert evaluation.json()["calibration_gap"] == pytest.approx(-0.3)
        empty_slice = client.get("/scientific/predictions/evaluation", params={"target": "risk", "horizon": "7d"}, headers=headers)
        assert empty_slice.status_code == 200
        assert empty_slice.json()["n"] == 0

        future = client.post("/scientific/predictions/outcomes", headers=headers, json={"prediction_id": "prediction-1", "decision_id": "decision-1", "action_id": "option-1", "outcome_id": "outcome-future", "target": "risk", "outcome_time": (now + timedelta(minutes=1)).isoformat(), "observed": 1, "provenance": ["test:future"]})
