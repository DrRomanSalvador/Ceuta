...
from app.integration.heat_risk import (
    API_URL,
    RISK_ID,
    VARIABLES,
    OpenMeteoHeatAdapter,
    integrate_heat_batch,
)
...
def payload(*, missing: str | None = None, invalid: str | None = None) -> dict:
    ...
    for index, variable in enumerate(VARIABLES):
        hourly[variable] = [
            None
            if variable == missing
            else ("bad" if variable == invalid else float(index + 1))
        ]
        units[variable] = "unit"
...
def test_real_adapter_parses_all_critical_variables() -> None:
    batch = OpenMeteoHeatAdapter.parse(
        payload(), latitude=35.8894, longitude=-5.3213
    )
...
def test_missing_variable_is_preserved_not_imputed() -> None:
    batch = OpenMeteoHeatAdapter.parse(
        payload(missing="uv_index"), latitude=35.8894, longitude=-5.3213
    )
...
def test_schema_or_type_failure_is_rejected() -> None:
    with pytest.raises(ValueError, match="Invalid numeric value"):
        OpenMeteoHeatAdapter.parse(
            payload(invalid="temperature_2m"),
            latitude=35.8894,
            longitude=-5.3213,
        )
...
def test_source_failure_propagates_without_fabricated_observation() -> None:
    transport = httpx.MockTransport(
        lambda request: httpx.Response(
            503, request=request, text="upstream unavailable"
        )
    )
    with httpx.Client(transport=transport) as client:
        with pytest.raises(httpx.HTTPStatusError):
            OpenMeteoHeatAdapter(client).ingest(
                latitude=35.8894, longitude=-5.3213
            )
...
def test_timestamp_is_aware_and_reproducible_from_source_payload() -> None:
    ...
    assert batch.observations[0].observed_at == datetime(
