...
def test_ingestion_admits_through_contract_and_preserves_unknown_uncertainty():
    ...
    result = BulkIngestionPipeline(registry).ingest_records([
        IngestionRecord(
            source_id="source-a",
            document_id="doc-a",
            statement="indicator=7",
            value=7,
            semantic_definition="indicator units",
            publication_time=T0.isoformat(),
        )
    ])
...
def test_ingestion_rejects_malformed_temporal_input_closed():
    ...
    result = BulkIngestionPipeline(registry).ingest_records([
        IngestionRecord(
            ...
            value=7,
            semantic_definition="indicator units",
            publication_time="not-a-timestamp",
        )
    ])
...
def test_two_independent_source_groups_upgrade_to_corrob_fact():
    ...
    registry.corroborate(
        target.evidence_id,
        corroborator.evidence_id,
